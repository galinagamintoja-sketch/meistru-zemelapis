...
export function PhotoUploader({ photos }: { photos: Photo[] }) {
  ...
  return <div className="portal-form">
    ...
    <input ref={pickerRef} className="visually-hidden-file-input" multiple type="file" disabled={isUploading} accept={REGISTRATION_PHOTO_ACCEPT} onChange={(event) => { void select(event.target.files); event.target.value = ""; }} />
