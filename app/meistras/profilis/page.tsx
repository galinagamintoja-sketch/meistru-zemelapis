...
export default async function Page() {
  ...
  const [{ data: categories }, { data: assignedCategories }, { data: selectedServices }, { data: savedRates }] = supabase ? await Promise.all([
    ...
    supabase.from("profile_services").select("service_category_id,service_subcategory_id,service_subcategories(name)").eq("tradesperson_profile_id", profile.id),
    supabase.from("profile_service_labour_rates").select("service_subcategory_id,amount").eq("tradesperson_profile_id", profile.id)
  ]) : [{ data: [] }, { data: [] }, { data: [] }, { data: [] }];
  const primaryCategoryId = resolveEditorCategoryId({
    ...
    serviceCategoryIds: (selectedServices ?? []).map((item) => item.service_category_id).filter(Boolean),
    legacyCategoryId: profile.service_category_id
  });
  const services = (selectedServices ?? []).flatMap((row) => {
    const relation = Array.isArray(row.service_subcategories) ? row.service_subcategories[0] : row.service_subcategories;
    return relation ? [{ id: row.service_subcategory_id, name: relation.name }] : [];
  });
  const rateByServiceId = new Map((savedRates ?? []).map((rate) => [rate.service_subcategory_id, rate.amount]));
  const checks = [
  ...
  return <div className="portal-page"><div className="portal-heading profile-edit-heading"><div><h1>Mano profilis</h1><p>Tvarkykite viešą profilį ir kontaktinius duomenis. Prisijungimo el. paštas keičiamas tik skiltyje „Paskyra“.</p></div>{isPublic ? <Link className="profile-public-link" href={`/specialist/${profile.id}`} target="_blank">Atidaryti viešą profilį</Link> : null}</div>
    ...
    <PortalCard title="Profilio informacija"><ProfileForm categories={categories ?? []} initial={{ displayName: profile.display_name, companyName: profile.company_name ?? "", primaryCategoryId, experienceYears: profile.experience_years ?? 0, phone: profile.phone, whatsappNumber: profile.whatsapp_number ?? "", publicEmail: profile.email, description: profile.description ?? "", languages: profile.languages ?? [], publicContactConsent: Boolean(profile.public_contact_consent_at), labourRateUnit: profile.labour_rate_unit ?? "agreed", labourRateAmount: profile.labour_rate_amount ?? null, selectedServices: services, serviceLabourRates: services.filter((service) => rateByServiceId.has(service.id)).map((service) => ({ serviceId: service.id, serviceName: service.name, amount: Number(rateByServiceId.get(service.id)) })) }} /></PortalCard></div>;
}
