...
export default async function Page() {
  ...
  return <div className="portal-page"><div className="portal-heading profile-edit-heading"><div><h1>Mano profilis</h1><p>Tvarkykite viešą profilį ir kontaktinius duomenis. Prisijungimo el. paštas keičiamas tik skiltyje „Paskyra“.</p></div><Link className="profile-public-link" href={`/specialist/${profile.id}`} target="_blank">Atidaryti viešą profilį</Link></div>
    <section className="profile-status-banner">
      <div className="profile-status-copy"><span className={`status-badge ${status === "Profilis aktyvus" ? "status-success" : "status-warning"}`}>{status}</span><p>{status === "Profilis aktyvus" ? "Jūsų profilis matomas meistrų žemėlapyje." : "Užbaikite trūkstamus profilio punktus."}</p></div>
