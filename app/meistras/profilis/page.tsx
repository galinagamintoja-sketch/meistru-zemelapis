...
export default async function Page() {
  ...
  return <div className="portal-page"><div className="portal-heading"><h1>Mano profilis</h1><p>Tvarkykite viešą profilį ir kontaktinius duomenis. Prisijungimo el. paštas keičiamas tik skiltyje „Paskyra“.</p></div>
    <section className="profile-status-banner">
      ...
      <Link className="portal-secondary" href={`/specialist/${profile.id}`} target="_blank">Peržiūrėti viešą profilį</Link>
      <div className="profile-checklist">{checks.map(([label, complete]) => <span data-complete={complete} key={label}>{complete ? "✓" : "○"} {label}</span>)}</div>
    </section>
