...
export default async function Page() {
  ...
  const [{ data: assignmentCategories, error: assignmentError }, { data: legacyCategories }, { data: current }, { data: currentCategories, error: currentCategoriesError }] = supabase ? await Promise.all([
    ...
    supabase.from("profile_services").select("service_subcategory_id,service_category_id").eq("tradesperson_profile_id", profile.id),
    supabase.from("profile_category_assignments").select("service_category_id").eq("tradesperson_profile_id", profile.id)
  ]) : [{ data: [], error: null }, { data: [] }, { data: [] }, { data: [], error: null }];
