...
const input = z.object({ action: z.enum(["sign-in", "sign-up", "recovery", "update-password"]), email: z.string().trim().email().optional(), password: z.string().min(10).max(128).optional() });
...
export async function POST(request: Request) {
  ...
  if (action === "sign-in") {
    ...
    return error ? NextResponse.json({ error: "El. paštas arba slaptažodis neteisingas." }, { status: 401 }) : NextResponse.json({ redirectTo: "/meistras" });
  }
  if (action === "sign-up") {
    if (!email || !password) return NextResponse.json({ error: "Įveskite el. paštą ir slaptažodį." }, { status: 400 });
    const { error } = await supabase.auth.signUp({ email, password, options: { emailRedirectTo: `${origin}/auth/callback?next=%2Fmeistras` } });
