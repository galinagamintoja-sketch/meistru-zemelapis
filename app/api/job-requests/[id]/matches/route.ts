...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  let { data, error } = await supabase.from("tradesperson_profiles").select(CANDIDATE_SELECT);
  if (error && /profile_category_assignments/i.test(error.message)) {
    ({ data, error } = await supabase.from("tradesperson_profiles").select(LEGACY_CANDIDATE_SELECT));
  }
