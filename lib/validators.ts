...
import { MAX_PROFILE_SERVICES, MAX_WORK_AREAS } from "./service-taxonomy";
...
export const registrationSchema = z.object({
  ...
  categorySlugs: z.array(z.string().trim().min(2).max(80)).max(MAX_WORK_AREAS).optional().default([]),
  subcategorySlugs: z.array(z.string().trim().min(2).max(80)).min(2, "Pasirinkite bent 2 paslaugas.").max(MAX_PROFILE_SERVICES).optional().default([]),
