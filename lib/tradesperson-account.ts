...
export async function getLinkedTradespersonProfile(authUserId: string) {
  ...
  const { data } = await supabase
    ...
    .select("id,display_name,company_name,phone,whatsapp_number,email,base_city,radius_km,registered_address,google_place_id,latitude,longitude,description,service_area_label,public_status,approval_status,service_category_id,experience_years,languages,public_contact_consent_at,labour_rate_unit,labour_rate_amount")
